...
class AccountFactoryTest extends TestCase
{
    ...
    protected string $seedText = "manage manual recall harvest series desert melt police rose hollow moral pledge kitten position add";
    ...
    public function convertSignToEncryptProvider()
    {
        return [
            ...
            [
                ['publickey' => "gVVExGUK4J5BsxwUfYsFkkjpn6A7BcvYdmARL28GBRc"],
                "EZa2ndj6h95m3xm7DxPQhrtANvhymNC7nWQ3o1vmDJ4x",
                null
            ],
            [
                ['publickey' => "FkU1XyfrCftc4pQKXCrrDyRLSnifX1SMvmx1CYiiyB3Y"],
                "BVv1ZuE3gKFa6krwWJQwEmrLYUESuUabNCXgYTmCoBt6",
                null
            ],
            [
                ['publickey' => "96yeNG1KYJKAVnfKqfkfktkXuPj1CLPEsgCDkm42VcaT"],
                "4Xpf8guEGD3ZnRJLuEu8JjpmKnHpXR49mFE4Zm9m9P1z",
                null
            ],
            [
                ['publickey' => "7TecQdLbPuxt3mWukbZ1g1dTZeA6rxgjMxfS9MRURaEP"],
                "Efv4wPdjfyVNvbp21xwiTXnirQti7jJy56W9doDVzfhG",
                null
            ],
            [
                [
            ...
            [
                ...
                null,
                "ACsYcMff8UPUc5dvuCMAkqZxcRTjXHMnCc29TZkWLQsZ"
            ],
            [
                [
                ...
                null,
                "BnjFJJarge15FiqcxrB7Mzt68nseBXXR4LQ54qFBsWHG"
            ]
        ];
    }
     ...
     */
    public function testConvertSignToEncrypt(array $sign, ?string $publicEncrypt, ?string $privateEncrypt)
    {
        $factory = new AccountFactory('L');

        $account = $factory->create(['sign' => $sign]);

        if ($publicEncrypt !== null) {
            $this->assertEqualsBase58($publicEncrypt, $account->encrypt->publickey);
        }
        if ($privateEncrypt !== null) {
            $this->assertEqualsBase58($privateEncrypt, $account->encrypt->secretkey);
        }
    }
